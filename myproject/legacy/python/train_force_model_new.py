...
def main():
    ...
    for ep in range(1, epochs_cd + 1):
        ...
        phi_tr = torch.nn.functional.softplus(b0 + b1 / (Re_cyl_tr + eps) + b2 / ((Re_cyl_tr + eps) ** 2))
        ...
        with torch.no_grad():
            ...
            phi_val = torch.nn.functional.softplus(b0 + b1 / (Re_cyl_val + eps) + b2 / ((Re_cyl_val + eps) ** 2))
    ...
    for ep in range(1, epochs_res + 1):
        ...
        phi_tr = torch.nn.functional.softplus(b0 + b1 / (Re_cyl_tr + 1e-6) + b2 / ((Re_cyl_tr + 1e-6) ** 2))
        m_tr = torch.nn.functional.softplus(m0)
        cd_soft_tr = 2.0 * phi_tr * torch.exp(-m_tr * torch.log1p(Ca_tr))
        ...
        with torch.no_grad():
            ...
            phi_val = torch.nn.functional.softplus(b0 + b1 / (Re_cyl_val + 1e-6) + b2 / ((Re_cyl_val + 1e-6) ** 2))
            m_val = torch.nn.functional.softplus(m0)
            cd_soft_val = 2.0 * phi_val * torch.exp(-m_val * torch.log1p(Ca_val))
    ...
    header = (
        "idx,v,Hc,Dc,L,Re_cyl,Re_cyl,Ca,Cd_cyl,Cd_soft,y_true,y_pred,F_cyl_pred,F_soft_pred,F_soft_pred_col1,F_soft_pred_col2,F_soft_pred_col3,Fc1(F@Cd=1),Fs1(F@Cd=1),Fs1_col1,Fs1_col2,Fs1_col3"
    )
    ...
    plt.scatter(Re_cyl_np, cd_s_all, s=18, label="Cd_soft vs Re_cyl")
    plt.xscale('log')
    plt.xlabel('Re')
    plt.ylabel('Cd')
    plt.legend()
